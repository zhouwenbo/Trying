...
public class MyFragmentTabHost extends TabHost implements TabHost.OnTabChangeListener {
    ...
    public static final class TabInfo {
        ...
        TabInfo(String _tag, Class<?> _class, Bundle _args, boolean _detach) {
            tag = _tag;
            clss = _class;
            args = _args;
            detach = _detach;
    ...
    public TabInfo getTabInfoByTagId(String tabId) {
...
//            ll.addView(tw, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0));
            addView(tw,new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.FILL_PARENT));
...
//        mRealTabContent.setId(containerId);
    ...
    public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args, boolean... detach) {
        tabSpec.setContent(new DummyTabFactory(mContext));
        String tag = tabSpec.getTag();
        boolean _detach = false;
        if (detach != null && detach.length > 0) {
            _detach = detach[0];
        }
        TabInfo info = new TabInfo(tag, clss, args, _detach);
    ...
    private void hideOrDetach(FragmentTransaction ft, TabInfo tab) {
        if (tab.detach) {
    ...
    private void showOrAttach(FragmentTransaction ft, TabInfo tab) {
        if (tab.detach) {
    ...
    public void onTabChanged(String tabId) {
        if (onTabClickListener != null && onTabClickListener.checkLogin(tabId)) { //校验是否登录

//	    	Lg.i("on tag change: " + tabId);
            if (mAttached) {
...
//			    	Lg.i("newTab fragment: " + newTab);
    ...
    public interface OnTabClickListener {
        ...
        boolean checkLogin(String tabId);

        void setCurrentTabByTag(String tabId);

        void setTabViewBageTips(int pos, int newMsgCount);

        void setCurrentTabByIndex(int pos);
    }
